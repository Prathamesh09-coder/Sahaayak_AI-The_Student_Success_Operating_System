...
// @ts-expect-error: react-gauge-chart does not publish TypeScript types
...
const SafeGaugeChart = (GaugeChart as any).default || GaugeChart;
...
function SuccessNavigator() {
      ...
      try {
        ...
        if (data.event) {
          ...
          if (data.event === "success.index_updated") {
            toast.info(`Success index re-calculated: ${data.payload.overall_score}%!`);
  ...
  const trends = trendsRes?.data || { weekly: { success_trend: [] }, monthly: { success_trend: [] }, yearly: { success_trend: [] } };
  const recommendations = recommendationsRes?.data || [];
  const predictions = predictionsRes?.data || { dropout_risk: 15, placement_probability: 75, financial_risk: 20, burnout_risk: 25, scholarship_probability: 80 };
  const forecast = forecastRes?.data || { "30_days": 80, "90_days": 85, "180_days": 90 };
    ...
    return (
      <div className="min-h-[70vh] grid place-items-center">
        <div className="text-center p-8 glass-strong rounded-3xl border border-border/50 max-w-md shadow-2xl">
          ...
          <h3 className="text-xl font-bold mb-2">Unable to load Success Navigator</h3>
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <header className="glass-strong shadow-soft relative overflow-hidden rounded-3xl p-6 md:p-8 border border-border/50">
        ...
        <div className="relative flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="text-xs uppercase font-semibold tracking-wider text-primary">Student Intelligence Center</p>
              <h1 className="text-2xl font-bold tracking-tight md:text-3xl mt-0.5">
                Success Navigator
              </h1>
              <p className="text-sm text-muted-foreground mt-1">
                Your continuous educational health report and predictive success analytics.
              </p>
            </div>
          </div>
          <div className="text-right shrink-0 bg-background/40 backdrop-blur-md border border-border/40 rounded-2xl p-3">
            ...
            <span className="text-xs font-bold text-foreground">{formatDate(index.last_updated)}</span>
          </div>
        </div>
      </header>
      ...
      {interventions.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-sm font-bold uppercase tracking-wider text-destructive flex items-center gap-2">
            <ShieldAlert className="size-4 animate-bounce" /> Active Intervention Actions Required
          </h3>
              ...
              >
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[8px] font-bold text-muted-foreground uppercase">Severity: {item.severity}</span>
                  </div>
                  <p className="text-xs font-semibold text-foreground mt-2">{item.reason}</p>
                  <p className="text-[11px] text-muted-foreground mt-1">Action: {item.recommended_action}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card title="Success Index Score" className="lg:col-span-1 flex flex-col items-center justify-center text-center">
          <div className="w-full max-w-[240px] mt-2 relative">
            <SafeGaugeChart
              id="success-overall-gauge"
              nrOfLevels={4}
              colors={["#ef4444", "#f97316", "#eab308", "#10b981"]}
              percent={index.overall_score / 100}
              arcWidth={0.25}
          ...
          <div className="text-5xl font-black text-primary mt-2 flex items-baseline justify-center">
            <SafeCountUp end={index.overall_score} decimals={1} duration={1.5} />
            <span className="text-2xl font-bold ml-0.5">%</span>
          </div>
          <div className="flex items-center gap-2 mt-3">
            <span className="font-extrabold text-lg text-emerald-400">{index.level} Standing</span>
          </div>
          <p className="text-xs text-muted-foreground mt-2 leading-relaxed max-w-[240px] mx-auto">
            Overall educational health computed dynamically across six intelligence metrics.
          </p>
        </Card>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
            ...
            {lineChartData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={lineChartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                  ...
                  <YAxis stroke="#64748b" domain={[40, 100]} style={{ fontSize: "10px" }} />
                  <Tooltip
                    contentStyle={{ background: "#0b0f19", border: "1px solid #1e293b", borderRadius: "12px" }}
        ...
        {/* Calendar Heatmap */}
        <Card title="Engagement Heatmap" className="lg:col-span-1 min-h-[360px]">
          <div className="mt-4 flex flex-col justify-between h-full pb-4">
            <span className="text-xs text-muted-foreground mb-4 block">
              Continuous student action log intensity mapping (logins, chats, roadmap completions).
            </span>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card title="Predictive Intelligence Engine" className="lg:col-span-1">
          <div className="space-y-4 max-h-[380px] overflow-y-auto pr-1">
            <h4 className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground mb-1">AI Future Readiness Predictions</h4>
        ...
        <Card title="Future Success Forecast" className="lg:col-span-2">
          <div className="flex justify-between items-start">
            <div>
              <h4 className="text-xs font-semibold text-foreground">Success Index Area Projections</h4>
              <p className="text-xs text-muted-foreground mt-0.5">Calculated based on your progress slope velocity.</p>
            </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={forecastChartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <Tooltip
                  ...
                  labelStyle={{ fontWeight: "bold" }}
                />
                <Area type="monotone" dataKey="Projection" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#colorProjection)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        ...
        <Card title="Why Did My Score Change? (Explainability)" icon={<AlertTriangle className="size-4 text-primary" />}>
            ...
            {explanations.map((item: any, idx: number) => (
              ...
              >
                <div className="flex justify-between items-center mb-1">
                  <span className="font-bold text-sm text-foreground">{item.factor}</span>
                  <span className={`text-[9px] font-black px-2 py-0.5 rounded-full uppercase border ${
                    item.impact === "High" ? "bg-red-500/10 text-red-400 border-red-500/20" : "bg-yellow-500/10 text-yellow-400 border-yellow-500/20"
                  }`}>
                    {item.impact} Impact
                  </span>
                </div>
                <p className="text-xs text-muted-foreground leading-relaxed mt-2">{item.reason}</p>
        ...
        <Card title="AI Success Optimization Plan" icon={<Sparkles className="size-4 text-primary" />}>
          <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className={`text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full border ${
                      rec.priority === "HIGH" ? "bg-red-500/15 text-red-400 border-red-500/20 shadow-glow" : "bg-yellow-500/10 text-yellow-400 border-yellow-500/20"
                    }`}>
                      {rec.priority}
                    </span>
                  </div>
                  <h4 className="font-bold text-sm mt-2 text-foreground truncate">{rec.title}</h4>
...
function ComponentScoreCard({ icon, label, score, weight, trend, status, color }: any) {
  return (
    <div className="bg-background/20 rounded-2xl p-4 border border-border/40 hover:border-border transition-all flex flex-col items-center text-center">
      <div className={`size-10 rounded-full bg-primary/10 ${color} grid place-items-center mb-2 shadow-glow`}>
        {icon}
      </div>
      <span className="text-xs font-bold text-foreground mb-1 leading-snug min-h-[32px] flex items-center justify-center">{label}</span>
...
function PredictiveProgressCard({ title, value, isSuccessChance, action }: { title: string; value: number; isSuccessChance: boolean; action: string }) {
  ...
  return (
    <div className="p-3 bg-background/30 rounded-2xl border border-border/40 hover:border-border transition-all">
      ...
      <div className="h-1.5 w-full rounded-full bg-muted/60 overflow-hidden">
        <div className={`h-full rounded-full transition-all duration-1000 ${barBg}`} style={{ width: `${value}%` }} />
      </div>
