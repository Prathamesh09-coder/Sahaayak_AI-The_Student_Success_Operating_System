...
const StudentNode = ({ data }: any) => (
  <div className="relative p-4 rounded-2xl border border-indigo-500/40 bg-background/70 backdrop-blur-md shadow-[0_0_25px_rgba(99,102,241,0.25)] text-center w-[190px]">
    ...
    <div className="font-bold text-sm truncate text-foreground">{data.name || "Student"}</div>
    ...
    <div className="grid grid-cols-2 gap-1 text-[9px] border-t border-border/50 pt-2 mt-1">
      <div>
        ...
        <span className="font-bold text-indigo-400">{data.success_score ? Math.round(data.success_score) : 75}%</span>
      </div>
      <div>
        <span className="text-muted-foreground block">Completeness</span>
        <span className="font-bold text-emerald-400">{data.profile_completeness ? Math.round(data.profile_completeness) : 80}%</span>
      </div>
    </div>

    <Handle type="source" position={Position.Top} id="s-top" className="!bg-indigo-500" />
    <Handle type="source" position={Position.Bottom} id="s-bottom" className="!bg-indigo-500" />
    <Handle type="source" position={Position.Left} id="s-left" className="!bg-indigo-500" />
    <Handle type="source" position={Position.Right} id="s-right" className="!bg-indigo-500" />
  </div>
);
  ...
  <div className="relative p-3 rounded-xl border border-blue-500/30 bg-background/70 backdrop-blur-md shadow-lg w-[160px] hover:border-blue-500 transition-all duration-300">
    <div className="flex items-center gap-2 mb-1">
      ...
      <div className="min-w-0 flex-1">
        <div className="font-bold text-xs truncate text-foreground">{data.name || "Dream Career"}</div>
...
const SkillNode = ({ data }: any) => (
  <div className="relative p-3 rounded-xl border border-emerald-500/30 bg-background/70 backdrop-blur-md shadow-lg w-[150px] hover:border-emerald-500 transition-all duration-300">
    <div className="flex items-center gap-2 mb-1">
      ...
      <div className="min-w-0 flex-1">
        <div className="font-bold text-xs truncate text-foreground">{data.name || "Skill"}</div>
...
const CourseNode = ({ data }: any) => (
  <div className="relative p-3 rounded-xl border border-amber-500/30 bg-background/70 backdrop-blur-md shadow-lg w-[170px] hover:border-amber-500 transition-all duration-300">
    <div className="flex items-center gap-2 mb-1">
      ...
      <div className="min-w-0 flex-1">
        <div className="font-bold text-xs truncate text-foreground" title={data.title}>{data.title || "Course"}</div>
        <div className="text-[9px] text-muted-foreground truncate">{data.provider || "Provider"}</div>
      </div>
    </div>
    <div className="flex items-center justify-between text-[9px] text-amber-400 mt-1">
      <span>{data.duration || "Self-paced"}</span>
      <span className="px-1.5 py-0.2 bg-amber-500/10 rounded">{data.difficulty || "Medium"}</span>
    </div>
...
const ScholarshipNode = ({ data }: any) => (
  <div className="relative p-3 rounded-xl border border-rose-500/30 bg-background/70 backdrop-blur-md shadow-lg w-[165px] hover:border-rose-500 transition-all duration-300">
    <div className="flex items-center gap-2 mb-1">
      ...
      <div className="min-w-0 flex-1">
        <div className="font-bold text-xs truncate text-foreground" title={data.title}>{data.title || "Scholarship"}</div>
...
const OpportunityNode = ({ data }: any) => (
  <div className="relative p-3 rounded-xl border border-purple-500/30 bg-background/70 backdrop-blur-md shadow-lg w-[160px] hover:border-purple-500 transition-all duration-300">
    <div className="flex items-center gap-2 mb-1">
      ...
      <div className="min-w-0 flex-1">
        <div className="font-bold text-xs truncate text-foreground" title={data.title}>{data.title || "Opportunity"}</div>
        <div className="text-[9px] text-muted-foreground truncate">{data.company || "Company"}</div>
      </div>
    </div>
    <div className="text-[9px] text-purple-400 mt-1">
      <span className="px-1.5 py-0.5 bg-purple-500/10 rounded">{data.type || "Internship"}</span>
    </div>
...
const MentorNode = ({ data }: any) => (
  <div className="relative p-3 rounded-xl border border-teal-500/30 bg-background/70 backdrop-blur-md shadow-lg w-[150px] hover:border-teal-500 transition-all duration-300">
    <div className="flex items-center gap-2 mb-1">
      ...
      <div className="min-w-0 flex-1">
        <div className="font-bold text-xs truncate text-foreground">{data.name || "Mentor"}</div>
        <div className="text-[9px] text-muted-foreground truncate">{data.company || "Company"}</div>
      </div>
    </div>
    <div className="text-[9px] text-teal-400 mt-1 flex justify-between">
      <span>Match:</span>
      <span className="font-bold">{data.match_score ? Math.round(data.match_score) : 85}%</span>
    </div>
...
const CommunityNode = ({ data }: any) => (
  <div className="relative p-3 rounded-xl border border-cyan-500/30 bg-background/70 backdrop-blur-md shadow-lg w-[150px] hover:border-cyan-500 transition-all duration-300">
    <div className="flex items-center gap-2 mb-1">
      ...
      <div className="min-w-0 flex-1">
        <div className="font-bold text-xs truncate text-foreground">{data.name || "Community"}</div>
...
const SuccessstoryNode = ({ data }: any) => (
  <div className="relative p-3 rounded-xl border border-yellow-500/30 bg-background/70 backdrop-blur-md shadow-lg w-[160px] hover:border-yellow-500 transition-all duration-300">
    <div className="flex items-center gap-2 mb-1">
      ...
      <div className="min-w-0 flex-1">
        <div className="font-bold text-xs truncate text-foreground" title={data.title}>{data.title || "Success Story"}</div>
...
function layoutGraph(rawNodes: any[], rawEdges: any[]) {
  ...
  outerNodes.forEach(node => {
    ...
    const edge = edges.find(
      e => (e.source === node.id && innerNodeIds.has(e.target)) || 
  ...
  Object.entries(parentToChildren).forEach(([parentId, children]) => {
    if (parentId === "default") {
      ...
      children.forEach((node, idx) => {
        ...
        const angleOffset = numChildren > 1 
...
const FILTER_OPTIONS = [
  { type: "career", label: "Careers", icon: Target, color: "text-blue-500 bg-blue-500/10 border-blue-500/20" },
  { type: "skill", label: "Skills", icon: Award, color: "text-emerald-500 bg-emerald-500/10 border-emerald-500/20" },
  { type: "course", label: "Courses", icon: BookOpen, color: "text-amber-500 bg-amber-500/10 border-amber-500/20" },
  { type: "scholarship", label: "Scholarships", icon: Gift, color: "text-rose-500 bg-rose-500/10 border-rose-500/20" },
  { type: "opportunity", label: "Opportunities", icon: Zap, color: "text-purple-500 bg-purple-500/10 border-purple-500/20" },
  { type: "mentor", label: "Mentors", icon: Users, color: "text-teal-500 bg-teal-500/10 border-teal-500/20" },
  { type: "community", label: "Communities", icon: Globe, color: "text-cyan-500 bg-cyan-500/10 border-cyan-500/20" },
  { type: "successstory", label: "Success Stories", icon: Trophy, color: "text-yellow-500 bg-yellow-500/10 border-yellow-500/20" }
];
...
function KnowledgeGraph() {
  ...
  const kpis = useMemo(() => {
    ...
    const acquiredSkills = rawEdges.filter((e) => e.label === "HAS_SKILL").length;
      ...
      <header className="glass-strong shadow-soft relative overflow-hidden rounded-3xl p-5 md:p-6 shrink-0 border border-white/10">
        ...
        <div className="relative flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Explore your personalized education, skills, and opportunity orbital ecosystem.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            >
              <RefreshCw className={`size-4 ${generating || isRefetching ? 'animate-spin' : ''}`} />
              Sync Graph
            </Button>
          </div>
        </div>
      ...
      {generating ? (
        <div className="flex-1 glass shadow-soft rounded-3xl relative overflow-hidden flex flex-col items-center justify-center bg-background/50 border border-white/5">
          ...
          <div className="max-w-md w-full text-center space-y-6 px-6 relative z-10">
            ...
            <div className="space-y-2">
              <h3 className="text-lg font-bold text-foreground">Synthesizing Success Universe</h3>
              <p className="text-xs text-muted-foreground max-w-xs mx-auto">
                Please wait while Sahaayak AI aligns your credentials, career aspirations, and milestones.
              </p>
            </div>
            ...
            <div className="space-y-2 text-left bg-muted/30 p-4 rounded-2xl border border-border/40">
              {LOADING_STEPS.map((step, idx) => (
                <div key={idx} className="flex items-center gap-3 text-xs">
                  <div className={`size-4 rounded-full flex items-center justify-center text-[8px] font-bold border ${
                    loadingStep > idx 
                  ...
                  <span className={`${loadingStep === idx ? 'text-indigo-400 font-semibold' : loadingStep > idx ? 'text-foreground/80' : 'text-muted-foreground'}`}>
                    {step}
                  </span>
                </div>
              ))}
        ...
        <div className="flex-1 glass shadow-soft rounded-3xl relative overflow-hidden flex flex-col items-center justify-center bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-indigo-500/5 via-background to-background border border-white/5 p-6 text-center">
          ...
          <div className="max-w-md space-y-6 relative z-10">
            ...
            <div className="space-y-2">
              <h2 className="text-2xl font-bold tracking-tight text-foreground">Your Success Universe is still forming</h2>
              <p className="text-xs text-muted-foreground leading-relaxed max-w-sm mx-auto">
                Your educational ecosystem connects your profile with career roadmaps, skill milestones, scholarships, and expert mentors. Run the synthesizer to chart your personalized universe.
              </p>
            </div>
        ...
        <div className="flex-1 flex gap-4 min-h-0 relative">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 shrink-0">
              <div className="glass shadow-soft p-3 rounded-2xl border border-white/5 hover:border-indigo-500/20 transition-all duration-300 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <span className="text-[10px] text-muted-foreground block truncate">Success Index</span>
              ...
              <div className="glass shadow-soft p-3 rounded-2xl border border-white/5 hover:border-blue-500/20 transition-all duration-300 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <span className="text-[10px] text-muted-foreground block truncate">Career Fit</span>
                  <span className="text-base font-bold text-foreground leading-tight">
              ...
              <div className="glass shadow-soft p-3 rounded-2xl border border-white/5 hover:border-emerald-500/20 transition-all duration-300 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <span className="text-[10px] text-muted-foreground block truncate">Skills Canopy</span>
                  <span className="text-base font-bold text-foreground leading-tight">
              ...
              <div className="glass shadow-soft p-3 rounded-2xl border border-white/5 hover:border-purple-500/20 transition-all duration-300 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <span className="text-[10px] text-muted-foreground block truncate">Growth Nodes</span>
                  <span className="text-base font-bold text-foreground leading-tight">
                  ...
                  return (
                    <Button
                      ...
                      className={`rounded-full text-[10px] h-8 gap-1.5 font-semibold px-4 ${
                        !isSelected ? 'hover:bg-background/80 border-border/40 bg-background/40' : ''
                      }`}
                    >
                      <Icon className="size-3.5" />
                      {opt.label}
                      <span className={`ml-1 px-1.5 py-0.2 rounded-full text-[8px] font-bold ${
            ...
            <div className="flex-1 glass shadow-soft rounded-3xl relative overflow-hidden bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-background/90 via-background to-muted/25 border border-white/5">
              ...
              <div className="absolute bottom-4 right-4 z-10">
                <div className="glass-strong p-3 rounded-2xl text-[9px] space-y-1.5 border border-white/10 shadow-xl max-w-[120px]">
                  <div className="font-semibold text-muted-foreground uppercase tracking-wider mb-1">Ecosystem</div>
                  <div className="flex items-center gap-1.5"><div className="size-2 rounded-full bg-indigo-500" /> Student</div>
                  <div className="flex items-center gap-1.5"><div className="size-2 rounded-full bg-blue-500" /> Career Goal</div>
                  <div className="flex items-center gap-1.5"><div className="size-2 rounded-full bg-emerald-500" /> Target Skills</div>
                  <div className="flex items-center gap-1.5"><div className="size-2 rounded-full bg-amber-500" /> Courses</div>
                  <div className="flex items-center gap-1.5"><div className="size-2 rounded-full bg-rose-500" /> Scholarships</div>
                  <div className="flex items-center gap-1.5"><div className="size-2 rounded-full bg-purple-500" /> Opportunities</div>
                  <div className="flex items-center gap-1.5"><div className="size-2 rounded-full bg-teal-500" /> Mentors</div>
                  <div className="flex items-center gap-1.5"><div className="size-2 rounded-full bg-cyan-500" /> Community</div>
                  <div className="flex items-center gap-1.5"><div className="size-2 rounded-full bg-yellow-500" /> Alumni Stories</div>
                </div>
              </div>
          ...
          <aside className="w-[340px] shrink-0 glass shadow-soft rounded-3xl flex flex-col border border-white/5 overflow-hidden">
            <div className="p-5 border-b border-border/40 bg-primary/5 flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-foreground">Sahaayak AI Insights</h3>
                <p className="text-[10px] text-muted-foreground">Ecosystem synthesis & paths</p>
              </div>
            </div>
                    ...
                    {insights.map((insight: any) => (
                      ...
                      >
                        <div className="mt-0.5 shrink-0">
                          {insight.type === 'academic' && <Award className="size-3.5 text-emerald-400" />}
                          {insight.type === 'career' && <Target className="size-3.5 text-blue-400" />}
                          {insight.type === 'scholarship' && <Coins className="size-3.5 text-rose-400" />}
                          {insight.type === 'mentor' && <Users className="size-3.5 text-teal-400" />}
                        </div>
                        <p>{insight.text}</p>
                      </div>
                    ))}
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <div className="space-y-0.5">
                            ...
                            <h5 className="text-xs font-bold text-foreground mt-1 leading-tight">{rec.title}</h5>
                          </div>
                        </div>
